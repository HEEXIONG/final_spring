...
public class PdController {
   ...
   public void ListGET(Model model, PdVo pdboard,FileVO filevo) {
	 ...
	 * 
	 * try{ String srcFileName = URLDecoder.decode(fileName,"UTF-8"); File file =
	 * new File(
	 * "C:\\Users\\hi-guro\\git\\Final_Boot2\\src\\main\\resources\\static\\Upload"
	 * + fileName); HttpHeaders header = new HttpHeaders();
	 * 
	 * //MIME 타입 처리 header.add("Content-Type",
	 * Files.probeContentType(file.toPath())); //File객체를 Path로 변환하여 MIME 타입을 판단하여
	 * HTTPHeaders의 Content-Type에 값으로 들어갑니다.
	 * 
	 * //파일 데이터 처리 *FileCopyUtils.copy 아래에 정리 //new
	 * ResponseEntity(body,header,status) result = new
	 * ResponseEntity<>(FileCopyUtils.copyToByteArray(file),header,HttpStatus.OK);
	 * 
	 * }catch (Exception e){ return new
	 * ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR); } return result; }
	 */
	 ...
	 * @GetMapping("/displayy")
	 * 
	 * @ResponseBody public ResponseEntity<byte[]> getFilee(String fileName) { File
	 * file = new File(
	 * "C:\\Users\\hi-guro\\git\\Final_Boot2\\src\\main\\resources\\static\\Upload"
	 * + fileName); ResponseEntity<byte[]> result = null; try { HttpHeaders header =
	 * new HttpHeaders(); header.add("Content-Type",
	 * Files.probeContentType(file.toPath())); result = new
	 * ResponseEntity<>(FileCopyUtils.copyToByteArray(file), header, HttpStatus.OK);
	 * } catch (Exception e) { e.printStackTrace(); } System.out.println("헤더 값이 뭐임?"
	 * + result.getHeaders()); return result; }
	 */
